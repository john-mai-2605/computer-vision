...
x_train = np.concatenate([X_train[:num_real], gen_imgs[:num_fake]], axis = 0)
y_train = np.concatenate([y_train[:num_real], sampled_labels[:num_fake]], axis = 0)
x_test = X_test
...
clf.compile(optimizer='adam',
              ...
              metrics=['accuracy'])
clf.load_weights('save_weight_classifier/normal/120_0_100.0.h5')
clf.compile(optimizer='adam',
              loss='sparse_categorical_crossentropy',
              metrics=['accuracy'])
...
new_path_save_model = 'save_weight_classifier/pretrain_then_real/' + str(100*perc_real) + '_' + str(100*perc_fake) + '_ver2.h5'
